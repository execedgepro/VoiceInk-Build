...
| BUG-26 | Gumroad zip originally built before Session 16 Gate 3 fixes — stale copy. Rebuilt, renamed, and finalised as EliteWrite-EarlyAccess-v0.4.10.zip (Jun 2 2026, v0.4.10-bug27-fix). CR-027 compliant. | High | 🔲 Open — ⚠️ LISTING RE-ARCHIVED Jun 2 2026 (Ferdz — no real buyers, test purchases only). Zip clean and ready. Reinstate only after 2 remaining gates: (1) Gate 5 — execedgepro.com/privacy live; (2) Ventura UTM test complete (Sonoma ✅ confirmed Jun 2 2026; BUG-27 ✅ Jun 2 2026). |
...
| CR-021 | macOS VERSION COVERAGE STANDARD — Development + all Phase 4 testing confirmed on macOS Tahoe (26.5) — PRIMARY. Sequoia (15.6.1) ✅ CONFIRMED via UTM VM, June 1 2026. Sonoma (14.6.1) ✅ CONFIRMED via UTM VM, Jun 2 2026. Ventura (13) pending UTM test. Monterey (12) NOT supported. Interim published statement updated to reflect Sonoma confirmed. Full compatibility statement must not be published without Rin sign-off per CR-022. Pre-launch gate: Ventura UTM test remaining. | High | Pre-launch gate |
...
| CR-027 | VERSION NAMING CONSISTENCY — The release zip filename, Gumroad content description, CLAUDE.md version log, and all public-facing references must always match the latest build version number. The build version number is the single source of truth. No exceptions. | High | Permanent |
| CR-028 | ZIP CONTENTS VERIFICATION — Every distribution zip must be verified immediately after creation using `unzip -l <zipfile>`. Verification must confirm: (1) single app file only; (2) correct app name; (3) correct version number; (4) no stale, duplicate, or golden copy files present. No zip may be uploaded to any distribution platform without this verification logged on record. Applies to all future builds — EliteWrite, EES iOS, EES macOS, EW Pro, all platforms. Triggered by BUG-29 — double app file (EliteWrite-Golden-v0.4.6.app) found in Gumroad zip during Sonoma UTM test June 2 2026. | High | Permanent |
...
*"Developed and tested on macOS Tahoe (26.5). Requires macOS Sonoma (14) or later. Sequoia (15) and Sonoma (14) — confirmed compatible. Ventura (13) — compatibility testing in progress via UTM."*
...
**Commercial model:** Locked. ⚠️ Gumroad listing RE-ARCHIVED Jun 2 2026 (no real buyers — test purchases only). Zip ready: EliteWrite-EarlyAccess-v0.4.10.zip (10MB, debug-excluded, CR-027/028 compliant). Reinstate after 2 gates: (1) Gate 5 privacy page live; (2) Ventura UTM test complete.
...
- BUG-26 ⚠️ Zip rebuilt clean (v0.4.10, 10MB, debug-excluded, CR-027/028 compliant). Re-archived — 2 gates to reinstate.
- BUG-27 ✅ Parakeet V2 default — OnboardingModelDownloadView rewritten to FluidAudioModelManager. Tester Rin Stage 5 confirmed.
- BUG-28 ✅ "Your Vibe-Coding Assistant" removed; TypewriterRoles 3 clean strings. Tester Rin Stage 5 confirmed.
- BUG-29 ✅ Double app in zip fixed — fresh zip creation protocol established.
- CR-027 ✅ Version Naming Consistency — sealed and enforced. v0.4.10 across all references.
- CR-028 ✅ ZIP Contents Verification — sealed. unzip -l mandatory before every upload.
- CR-021 ✅ Sonoma 14.6.1 confirmed via UTM VM. Screen Recording = 3rd required permission documented.
- Council roster ✅ 10-member Claude.ai Council + 6-member CC Team defined.
- CLAUDE.md ✅ v2.24 final.
